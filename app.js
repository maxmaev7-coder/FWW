...
async function buildPrintSheet() {
  ...
  // Small: 5 columns x 4 rows = 20? Or 5x5=25? 
  // User asked for more columns. Let's try 5 columns.
  // Large: 3 columns x 3 rows = 9?
  const perPage = variant === 'small' ? 25 : 9
  ...
  allEntries.forEach((entry) => {
    if (entry.separator) {
      ...
      sep.className = 'pdf-separator'
      grid.appendChild(sep)
      return
    }

    if (entry.header) return // legacy
    ...
    if (entry.item) {
      ...
      forceVertical = true; // Units always vertical
    }

    flagCardOrientation(img, cell, forceVertical)
  ...
  // ... (rest of function is usually error handling or cleanup, can skip or keep)
...
function createPrintEntryFromCard(entry) {
  ...
  // Store item ref so sorting works!
  return {
    img: entry.item.img,
    fallback: 'images/missing-item.png',
    mods,
    item: entry.item,
    isUnit: !!entry.item.unit // Identify if it came from a unit definition
  }
}
